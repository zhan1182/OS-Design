...
void ProcessFreeResources (PCB *pcb) {
  ...
  page = (uint32)(pcb->sysStackPtr) >> MEM_L2FIELD_FIRST_BITNUM;
  pcb->sysStackArea = 0;
  MemoryFreePage(page);
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  /* printf("about to allocate physical page\n"); */

  physical_page_number = MemoryAllocPage();
  
  /* printf("physical_page_number = %d\n", physical_page_number); */
  ...
  /* setup_l2_pte(MemorySetupPte(MemoryAllocPage()), pcb->pagetable[MEM_L1_PAGE_TABLE_SIZE - 1], MEM_L2_PAGE_TABLE_SIZE - 1); */
  ...
  /* setup_l2_pte(MemorySetupPte(MemoryAllocPage()), pcb->pagetable[0], 0); */
  /* setup_l2_pte(MemorySetupPte(MemoryAllocPage()), pcb->pagetable[0], 1); */
  ...
  if (isUser) {
    ...
    /* printf("stackframe[PROCESS_STACK_USER_STACKPOINTER] = %x\n", stackframe[PROCESS_STACK_USER_STACKPOINTER]); */
...
void main (int argc, char *argv[])
{
  ...
  /* printf ("Set timer quantum to %d, about to run first process.\n", */
  /* 	    processQuantum); */
